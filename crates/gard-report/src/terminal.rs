...
pub fn print_results(results: &[PackageResult], ctx: &RuntimeContext) {
    match ctx {
        ...
        RuntimeContext::ClaudeCode
        | RuntimeContext::Cursor               => print_compact(results),
...
pub fn print_init_success(ci_detected: bool, ci_name: Option<&str>) {
    eprintln!();
    eprintln!("  {}  v{}  SIGN · SCAN · PROTECT",
        style("gard").green().bold(),
        env!("CARGO_PKG_VERSION"));
    eprintln!();
    eprintln!("  {} git hooks installed   (pre-push)", style("✓").green());
    eprintln!("  {} manifest created      (.gard/manifest.json)", style("✓").green());
    eprintln!("  {} config written        (.gard/config.toml)", style("✓").green());
    if ci_detected {
        eprintln!();
    ...
    eprintln!();
    eprintln!("  {}  gard is protecting this repo — local + remote.", style("🛡").green());
    eprintln!();
}
...
fn print_standard(results: &[PackageResult]) {
    ...
    let blocked: Vec<_> = results.iter().filter(|r| r.verdict == Verdict::Block).collect();
    let warned:  Vec<_> = results.iter().filter(|r| r.verdict == Verdict::Warn).collect();
    ...
    if !blocked.is_empty() {
        ...
        eprintln!();
        eprintln!("  {} run `gard explain {}` for details",
            style("💡").yellow(), blocked[0].package.name);
        eprintln!("     run `gard allow {}` to override (not recommended)",
            blocked[0].package.name);
    } else if !warned.is_empty() {
        eprintln!("  {} manifest updated → .gard/", style("✍").cyan());
        eprintln!("  → push allowed with {} warning{}",
            warned.len(), if warned.len() == 1 { "" } else { "s" });
    } else {
        eprintln!("  {} manifest updated → .gard/", style("✍").cyan());
...
fn print_package_row(r: &PackageResult) {
    ...
    match r.verdict {
        Verdict::Pass  => eprintln!("  {}  {}  {}",
            style("✓").green(), style(name_col).white(), style(info).dim()),
        Verdict::Warn  => eprintln!("  {}  {}  {}",
            style("⚠").yellow(), style(name_col).yellow(), style(info).dim()),
        Verdict::Block => eprintln!("  {}  {}  {}",
            style("🚨").red(), style(name_col).red().bold(), style(info).red()),
    }
}
...
fn print_ci(results: &[PackageResult]) {
    for r in results {
        ...
        match &r.verdict {
            Verdict::Block => {
                let reason = block_reason(r);
                eprintln!("::error file={lockfile}::gard: {} BLOCKED — {reason}",
                    r.package);
            }
            Verdict::Warn => {
                eprintln!("::warning file={lockfile}::gard: {} WARN (score {})",
    ...
    eprintln!("[gard] {} scanned  {} blocked  {} warned",
        results.len(), blocked, warned);
}
