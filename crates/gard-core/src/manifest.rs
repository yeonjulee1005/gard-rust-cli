...
impl Manifest {
    ...
    pub fn upsert(&mut self, result: &PackageResult, added_by: Option<String>) {
        ...
        if let Some(pos) = self.packages.iter().position(|e| {
            e.name == result.package.name && e.ecosystem == result.package.ecosystem
        }) {
...
impl TierSummary {
    fn from_tier(t: &TierResult) -> Self {
        match t {
            TierResult::Pass              => Self { result: "PASS".into(),    reason: None },
            TierResult::Skipped           => Self { result: "SKIPPED".into(), reason: None },
            TierResult::Warn { reason }   => Self { result: "WARN".into(),    reason: Some(reason.clone()) },
            TierResult::Block { reason }  => Self { result: "BLOCK".into(),   reason: Some(reason.clone()) },
            TierResult::Error { message } => Self { result: "ERROR".into(),   reason: Some(message.clone()) },
        }
    }
}
