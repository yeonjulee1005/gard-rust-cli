...
pub fn find_new_packages(repo_root: &Path, manifest: &Manifest) -> anyhow::Result<Vec<Package>> {
    ...
    let checked: HashSet<(String, String)> = manifest.packages.iter()
...
pub fn diff_packages(before: &[Package], after: &[Package]) -> Vec<Package> {
    let before_set: HashSet<(&str, &str)> = before.iter()
...
pub fn parse_cargo_packages(repo_root: &Path) -> anyhow::Result<Vec<Package>> {
    ...
    if lock_path.exists() {
        ...
        return Ok(lock.package.unwrap_or_default().into_iter()
    ...
    let mut packages = Vec::new();
    for deps in [cargo.dependencies, cargo.dev_dependencies].into_iter().flatten() {
        for (name, val) in deps {
            packages.push(Package::new(name, extract_cargo_version(&val), Ecosystem::Cargo));
        }
    }
    Ok(packages)
}
...
fn normalize_version(spec: &str) -> String {
    ...
    let version = stripped.split(|c| c == ',' || c == ' ')
        .next()
        .unwrap_or("")
        .trim();
...
mod tests {
    ...
    fn parse_npm_package_json() {
        ...
        assert!(pkgs.iter().any(|p| p.name == "axios"  && p.version == "1.7.0"));
    ...
    fn parse_requirements_txt() {
        ...
        assert_eq!(pkgs.len(), 3);
        assert!(pkgs.iter().any(|p| p.name == "requests" && p.version == "2.28.0"));
        assert!(pkgs.iter().any(|p| p.name == "django"   && p.version == "4.0"));
        assert!(pkgs.iter().any(|p| p.name == "flask"    && p.version == "2.0.0"));
    }
